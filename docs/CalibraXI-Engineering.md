...
- MinIO remains the raw evidence and metadata store. A live smoke path exercised the governed bucket and returned 145 objects; PostgreSQL smoke counts include 12,704 fixtures, 62 Feature Snapshots, two training datasets, four model artifacts, two calibration artifacts, and two Forecast Runs.
...
- Historical Football-Data publication/availability chronology is unknown, ESPN historical fixture mappings are not yet measured in the bootstrap population, and player, lineup, event, shot, team-match-stat, and Understat xG/xA populations remain unsupported or missing. These states are persisted and are release gates for production PIT forecasting rather than hidden fallbacks.
