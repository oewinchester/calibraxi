...
### 15.2 2026-09-25 - Data Bootstrap and Forecasting Foundation

- The initial canonical EPL dataset is persisted through the governed acquisition boundary. It covers 33 seasons (`1993/94` through `2025/26`), 12,704 completed fixtures, and 51 teams. The coverage report retains explicit unsupported, missing, quarantined, and PIT-ineligible populations rather than converting them to zeroes.
- The first feature schema is `features-v1`. A Feature Snapshot is immutable and keyed by fixture, context, cutoff, knowledge time, schema version, evidence IDs, and per-feature evidence lineage. Training consumes versioned snapshots and never reconstructs history from mutable current tables.
- PIT eligibility requires event time, fixture kickoff, source observation/availability where known, CalibraXI knowledge time, and processing time to satisfy the cutoff rules. Unknown historical source availability is `PIT_INELIGIBLE`; it is not inferred from retrieval time.
- The initial coherent score distribution produces home/draw/away, total-goal, both-teams-to-score, and over/under probabilities from one normalized score matrix. Offline baselines are Frequency, Poisson, and Elo; comparison is deterministic and chronological.
...
- Temperature calibration is measured separately and versioned with its training window. It worsened synthetic raw log loss, so no calibration or model is promoted to production from that evidence. Actual-knowledge historical EPL PIT remains withheld where source availability chronology is unknown; the separate event-derived reconstruction evaluation is research evidence and does not claim historical CalibraXI knowledge.
...
- The raw reconciliation report retained 158 kickoff-only differences. The explicit Europe/London audit reclassified 156 as timezone-normalization artifacts and leaves two residual kickoff schedule-revision candidates (`+10` and `+15` minutes). No score, team-pair, competition, season, or duplicate-canonical-fixture conflict was promoted away.
...
### 15.4 2026-09-25 - Actual versus reconstructed historical PIT

- `EligibilityBasis` is explicit on canonical match records and Feature Snapshots: `actual_source_timestamp`, `calibraxi_knowledge_timestamp`, `revision_timestamp`, `event_derived_reconstruction`, and `unknown`. `unknown` is always PIT-ineligible.
- Event-derived reconstruction is limited to immutable facts from completed prior fixtures: fixture identity, competition/season, final result/score, W/D/L, goals for/against, rolling form, rest, and a replayed pre-match Elo state. It never claims that CalibraXI knew the fact at the historical time.
- The conservative default boundary is kickoff plus three hours when a source-local time exists and kickoff plus thirty hours when it does not. Provider statistics, xG/xA, shots, lineups, player data, injuries, odds, and market values require their own chronology evidence and are excluded from this result-only release.
- The first real evaluation is an expanding-window walk-forward with a strict fixture-kickoff cutoff, a 380-example warm-up, a shared deterministic `features-v2-real-pit-r2`/`real-pit-dataset-v4` population, immutable per-prediction snapshots, per-season metrics, reliability bins, skip reasons, replayable power-rating history, and selection evidence that records the feature schema, training/evaluation windows, and pooled/season metrics. Elo is an offline candidate; no runtime model shopping or Recommendation layer is introduced.

### 15.5 2026-09-26 - Live shadow and true-PIT prospective collection

- Prospective evidence is recorded in an append-only knowledge ledger. `knowledge_at` is the time CalibraXI actually retrieved and retained an observation; provider event/update timestamps remain source metadata and never substitute for CalibraXI knowledge time.
- The governed collection horizons are `fixture_first_observed`, `t-72h`, `t-24h`, `t-6h`, `t-1h`, `t-15m`, plus explicit event-driven observations. A task that was not collected at its scheduled window records `missed_observation`. A later fetch cannot backfill that missed horizon or make the observation eligible for an earlier cutoff.
- Prospective Feature Snapshots are immutable, versioned, and eligible only when every consumed observation has `knowledge_at <= cutoff_at`. Late lineups, corrected kickoff details, provider statistics, odds, and other unknown-chronology values remain missing for earlier snapshots. Provider-specific metrics retain provider identity; Sofascore xG and Understat xG are not interchangeable.
- `SHADOW` is an operating/publication state for real pre-match forecasts. Each run preserves the feature-information `cutoff_at` and `knowledge_at` from its immutable Feature Snapshot; the store separately assigns `prediction_cutoff_at` and `persisted_at` when the forecast is durably staged and attested. These timestamps cannot be used to move the feature cutoff forward. Runs must be persisted before kickoff and carry fixture, snapshot/schema, model/version, power state, raw coherent score distribution, calibration state, and evidence lineage. They are not Signals, Recommendations, or betting artifacts.
- Settlement is append-only. A final result creates a Settlement and Track Record entry only after the shared TRUE-PIT eligibility check proves `knowledge_at <= cutoff_at`, `created_at <= prediction_cutoff_at`, `persisted_at <= prediction_cutoff_at < kickoff_at`, and actual fixture-source lineage. `prediction_cutoff_at` and `persisted_at` must also be no later than the evaluation's as-of time. Rows from older schemas with no store-assigned prediction cutoff remain audit-only and are not upgraded into the prospective population. A corrected result creates a new settlement linked through `correction_of` and supersedes the prior row for current aggregates without deleting audit history. Prospective Track Record is kept separate from `HISTORICAL_RECONSTRUCTED` evaluation.
- A completed fixture projection does not establish result chronology through `updated_at`; it is eligible only with explicit source `knowledge_at`. An explicit result ledger observation may qualify the fixture using its own actual knowledge time.
- Reliability and monitoring are measurement artifacts, not product Signals. Reports expose `insufficient_sample`, `provisional`, or `measured` status with bin counts and uncertainty intervals. No model is automatically retrained, promoted, or replaced from a short live run; offline governance remains the promotion authority.
- The live runner executes Frequency, Poisson, Dixon-Coles, and the historically selected Elo candidate as independent shadow challengers on equivalent PIT-valid inputs. ESPN schedule/detail and Sofascore lineup/incident/team-player-stat observations are retained with actual knowledge time when their provider identities are available. Understat xG/xA remain explicit unsupported observations until a permitted runtime adapter and provider fixture mapping exist; no provider metric is silently substituted or added to the governed Elo feature schema. No qualified prospective odds source is configured.
- Prospective Track Record admission is versioned as `prospective-true-pit-v2` and requires a successful fixture-source observation in the forecast's immutable snapshot lineage, with matching fixture/evidence IDs and `knowledge_at <= cutoff_at`. The store-assigned `prediction_cutoff_at` proves the run's durable availability separately from the feature cutoff; `persisted_at` and prediction cutoff must both precede kickoff and the report's `generated_at`. Existing rows without the new store-assigned cutoff remain audit-only and are not silently included in v2.
- Timestamp-semantics clarification: the original live-shadow wording treated the feature cutoff and persistence cutoff as one field. This version supersedes that interpretation: `cutoff_at` remains the immutable source-information boundary, while `prediction_cutoff_at` records when the store could attest the run. The migration leaves this field null on legacy rows so replayed forecasts cannot be promoted retrospectively.
